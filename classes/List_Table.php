...
class List_Table extends WP_List_Table
{
    ...
    public function atlas_res($rows)
    {
    ...
    public function column_ostan($item)
    {
        ...
        $provinces = atlas_remote('https://api.mrrashidpour.com/iran/provinces.json');
    ...
    {
        return sprintf('<img src="%s" style="height:50px;">', atlas_panel_image('avatar/' . $item[ 'avatar' ] . '.jpg'));
    }
    ...
    public function column_atlas_row($item)
    {
        $this->m++;
        return $this->m;
    }
    public function column_atlas_update($item)
    {

        $atlas_update = '
            <button data-id="'.$item['ID'].'" data-type="successful" class="button button-primary atlas_update_row">تایید امضا</button>
            <button data-id="'.$item['ID'].'" data-type="failed" class="button action atlas_update_row">رد امضا</button>
            <button data-id="'.$item['ID'].'" data-type="delete" class="button button-primary button-error atlas_update_row">حذف امضا</button>
        ';
