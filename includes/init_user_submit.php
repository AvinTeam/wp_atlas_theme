...
function atlas_user_submit_init()
{
    ...
    if (isset($_POST[ 'act_user' ])) {
                ...
                if ($query->have_posts()) {
                    ...
                    if ($post_update) {
                        ...
                        if (! empty($_FILES[ 'fileInput' ][ 'name' ])) {
                            ...
                            $upload = atlas_upload_file($_FILES[ 'fileInput' ], absint($attachment_id), $post_id);
        ...
        if (isset($_REQUEST[ 'post' ]) && ! absint($_POST[ 'post_id' ]) && $_POST[ 'act_user' ] == 'form_submit') {
            ...
            if (isset($_POST[ '_wpnonce' ]) &&
                ...
                $post_id = wp_insert_post([
                    ...
                    'post_excerpt' => sanitize_textarea_field($_POST[ 'post_content' ]),
                    'post_status'  => 'pending',
                    'post_type'    => 'institute',
                ...
                if (! is_wp_error($post_id)) {
                    ...
                    update_post_meta($post_id, '_atlas_responsible', $this_user->nickname);
                    update_post_meta($post_id, '_atlas_responsible-mobile', $this_user->mobile);
        ...
        if (isset($_REQUEST[ 'profile' ]) && $_POST[ 'act_user' ] == 'form_submit_profile') {
            if (wp_verify_nonce($_POST[ '_wpnonce' ], 'atlas_nonce_user_submit' . get_current_user_id())) {
                $this_user = wp_get_current_user();
