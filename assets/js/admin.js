document.querySelectorAll('.delete-city-btn').forEach(button => {
    button.addEventListener('click', function () {
        ...
        const confirmation = confirm('آیا از پاک کردن این شهر مطمئن هستید؟');

        if (confirmation) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', atlas_js.ajax_url, true);
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    ...
                    if (response.success) {
                        ...
                        alert(response.data || 'خطایی رخ داد. دوباره امتحان کنید.');
                    }
                } else {
                    alert('خطای سرور. دوباره امتحان کنید.');
                }
            };
            xhr.send(`action=atlas_delete_city&nonce=${atlas_js.nonce}&city_id=${cityId}`);
        }
    });
});
